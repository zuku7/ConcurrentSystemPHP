...
<?php $tasks = $project->tasks ?>
	$(function() {<?php echo '"use strict";' . PHP_EOL;
echo '$(".gantt").gantt({' . PHP_EOL;
echo 'source: [{' . PHP_EOL;
echo 'name: " ",' . PHP_EOL;
echo 'desc: "' . $tasks[0] -> name . '",' . PHP_EOL;
echo 'values: [{' . PHP_EOL;
$start1 = strtotime($tasks[0] -> start);
$end1 = strtotime($tasks [0] -> end);
$start1 *= 1000;
$end1 *= 1000;
echo 'from: "/Date(' . $start1 . ')/",' . PHP_EOL;
echo 'to: "/Date(' . $end1 . ')/",' . PHP_EOL;
echo 'label: "' . $tasks[0] -> name . '",' . PHP_EOL;
